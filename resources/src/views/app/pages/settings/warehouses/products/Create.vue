    ...
    <div v-else>
      ...
      >
        <div slot="selected-row-actions">
          <button class="btn btn-primary" @click="storeProduct()">
            {{$t('Add')}}
          </button>
        </div>
        <div slot="table-actions" class="mt-2 mb-3">
          ...
          <b-button @click="productsPDF()" size="sm" variant="outline-success m-1">
          ...
          <b-button
            ...
            size="sm"
            variant="info m-1"
            v-if="currentUserPermissions?.includes('product_import')"
          >
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            ...
            <a v-if="currentUserPermissions?.includes('products_add')"
              v-b-tooltip.hover
              title="View"
              class="cursor-pointer"
              @click="storeProduct()"
            >
              <i class="i-Add text-25 text-info"></i>
            </a>
          </span>
...
export default {
  ...
  methods: {
    ...
    storeProduct() {
      axios.post(`warehouses/${this.$route.params.warehouse}/products`, {
    ...
    Submit_import() {
      ...
      axios.post("products/import/csv", self.data).then(response => {
    ...
    selectionChanged({ selectedRows }) {
      ...
      selectedRows.forEach((row, index) => {
        this.selectedProducts.push(row.id);
      });
    },
    ...
    destroyProduct(id) {
      this.$swal({
        ...
        if (result.value) {
          ...
          axios.delete(`/warehouses/${warehouse}/products/${product}`).then(() => {
            this.$swal(
