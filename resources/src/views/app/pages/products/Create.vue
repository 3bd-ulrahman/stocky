<template>
  <div class="main-content">
    ...
    <validation-observer ref="storeProduct" v-if="!isLoading">
      <b-form @submit.prevent="storeProduct" enctype="multipart/form-data">
        <b-row>
                  ...
                  <validation-provider name="Tax Method" :rules="{ required: true}">
                    ...
                    >
                      <v-select multiple :close-on-select="false"
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        v-model="product.warehouses_ids"
                        :reduce="label => label.value"
                  ...
                  >
                    <b-form-group :label="$t('StockAlert')">
                      <b-form-input
                        ...
                        v-model.number="product.stock_alert"
                        type="number"
                      />
                      <b-form-invalid-feedback
                    ...
                    <table class="table table-hover table-sm">
                      ...
                      <tbody>
                        ...
                        <tr v-for="variant in variants">
                          <td>
                            <input class="form-control" v-model="variant.code">
                          </td>
                          <td>
                            <input  class="form-control" v-model="variant.text">
                          </td>
                          <td>
                            <input class="form-control" v-model="variant.cost">
                          </td>
                          <td>
                            <input class="form-control" v-model="variant.price">
                          </td>
                          <td>
                            <a style="color: #ffff;"
...
export default {
  ...
  methods: {
    ...
    storeProduct() {
      this.$refs.storeProduct.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
    ...
    storeProduct() {
      ...
      // select all warehouses if not is selected
      if (this.product.warehouses_ids.length == 0) {
        this.product.warehouses_ids = this.warehouses.map(
      ...
      const payload = JSON.stringify(Object.fromEntries(
        Object.entries(this.product).map(([key, value]) => [key, value])
      ));
      ...
      if (this.images.length > 0) {
        for (var k = 0; k < this.images.length; k++) {
          Object.entries(this.images[k]).forEach(([key, value]) => {
            formData.append("images[" + k + "][" + key + "]", value);
          });
        }
      }
